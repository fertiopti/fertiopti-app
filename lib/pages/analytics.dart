...
class AnalyticsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: sensorDataList.isEmpty
          ? Center(child: Text("No data available for today."))
          : LayoutBuilder(
  ...
  Widget _buildSummary(List<SensorDataModel> data) {
    if (data.isEmpty) {
      return Center(child: Text("No data available to summarize."));
    }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text("Summary for Today", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          SizedBox(height: 8),
          _buildSummaryRow("Soil Moisture", latestData.soilMoisture.toStringAsFixed(2), averages['avgSoilMoisture']!.toStringAsFixed(2), Color(0xFFC8E6C9)),
          _buildSummaryRow("Temperature", latestData.temperature.toStringAsFixed(2), averages['avgTemperature']!.toStringAsFixed(2), Color(0xFFFFF9C4)),
          _buildSummaryRow("Humidity", latestData.humidity.toStringAsFixed(2), averages['avgHumidity']!.toStringAsFixed(2), Color(0xFFE0F7FA)),
        ],
      ),
    );
  }
